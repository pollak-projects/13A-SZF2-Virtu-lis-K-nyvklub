...
import creativeService from "../../services/misc/creative.service.js";
...
creativeRouter.post("/creatives", async (req, res) => {
  try {
    const data = req.body;
    const newCreative = await creativeService.createCreative(data);
...
creativeRouter.put("/creatives/:id", async (req, res) => {
  try {
    const { id } = req.params;
    const data = req.body;
    const updatedCreative = await creativeService.updateCreative(id, data);
